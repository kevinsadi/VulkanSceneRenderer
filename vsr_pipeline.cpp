...
namespace vsr {
	...
	VsrPipeline::~VsrPipeline() {
		vkDestroyShaderModule(vsrDevice.device(), vertShaderModule, nullptr);
		vkDestroyShaderModule(vsrDevice.device(), fragShaderModule, nullptr);
	...
	void VsrPipeline::createGraphicsPipeline(
		...
		assert( 
			...
			"Cannot create graphics pipeline: configInfo.pipelineLayout not supplied");
		assert(
			configInfo.renderPass != VK_NULL_HANDLE &&
			"Cannot create graphics pipeline: configInfo.renderPass not supplied");
		...
		VkPipelineShaderStageCreateInfo shaderStages[2]; // define two stages of creating info
		// vertex shader
		...
		VkPipelineVertexInputStateCreateInfo vertexInputInfo{}; // struct is value initialized to 0 by default
		...
		if (vkCreateGraphicsPipelines(vsrDevice.device(), VK_NULL_HANDLE, 1, &pipelineInfo, nullptr, &graphicsPipeline) != VK_SUCCESS)
		{
	...
	void VsrPipeline::createShaderModule(const std::vector<char>& code, VkShaderModule* shaderModule) {
		...
		// Vulkan methods only take in pointers
